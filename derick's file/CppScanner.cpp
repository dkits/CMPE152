...
bool CppScanner::skip_white_space() throw (string)
{
    char current_ch = current_char();
    ...
    while (isspace(current_ch) || (current_ch == '/')) {
    	...
    	if(current_ch == '/'){
			...
			if (current_ch == '/')
			{
				...
				// Found closing '}'?
				if (current_ch == Source::END_OF_LINE)
				{
			...
			{
	//        	current_ch = next_char(); // comsumes the *
				do
				{
					current_ch = next_char();  // consume comment characters
					if (current_ch == '*')
        ...
        // Not a comment.
        	...
        	{
        	current_ch = next_char();  // consume whitespace character
        	}
    }
    return false;
}
